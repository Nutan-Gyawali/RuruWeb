...
export const Footer = ({ language }: { language: Language }) => {
    return (
        <footer className="mt-16 rounded-t-[var(--radius-page)] bg-ink px-6 pt-12 pb-0 text-paper/70">
            <div className="mx-auto grid max-w-7xl grid-cols-2 gap-10 sm:grid-cols-4">
                ...
                <div>
                    <h3 className="mb-3 text-xs font-semibold uppercase tracking-[0.15em] text-sage-on-ink">{language === 'ne' ? 'सम्पर्क' : 'Contact'}</h3>
                ...
                <div>
                    <h3 className="mb-3 text-xs font-semibold uppercase tracking-[0.15em] text-sage-on-ink">{language === 'ne' ? 'समुदाय' : 'Community'}</h3>
                    <ul className="space-y-1.5 text-sm">
            ...
            {/* Map */}
            <div className="mx-auto mt-10 max-w-7xl border-t border-white/10 pt-10">
                <div className="mb-3 flex items-center gap-2">
                    <MapPin className="h-4 w-4 text-brand-on-ink" />
                    <h3 className="text-xs font-semibold uppercase tracking-[0.15em] text-brand-on-ink">{language === 'ne' ? 'स्थान' : 'Location'}</h3>
                </div>
                <iframe
                    title="Thorga map"
                    src="https://www.google.com/maps?q=Thorga,Gulmi,Nepal&z=13&output=embed"
                    loading="lazy"
                    className="h-48 w-48 rounded-[var(--radius-container)] saturate-[0.85]"
                />
            </div>
