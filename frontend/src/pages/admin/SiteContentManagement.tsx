        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                ...
                <button
                    ...
                    className="flex items-center gap-2 rounded bg-brand px-4 py-2 text-sm font-medium text-brand-on hover:bg-brand-muted transition-colors"
                >
                            ...
                            {items.map(item => (
                                <tr key={item.id} className="hover:bg-paper-muted/50 transition-colors">
                                    ...
                                    <td className="px-4 py-3 text-right">
                                        <button onClick={() => handleOpenForm(item)} className="p-1.5 text-brand hover:bg-[var(--color-brand-muted)] rounded mr-2 inline-block">
                                            <Edit2 className="h-4 w-4" />
                                        </button>
                                        <button onClick={() => handleDelete(item.id)} className="p-1.5 text-[var(--color-on-warning)] hover:bg-[var(--color-warning-muted)] rounded inline-block">
                        ...
                        <form onSubmit={handleSubmit} className="p-4 space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <input required type="number" value={formData.sortOrder || 0} onChange={e => setFormData({ ...formData, sortOrder: parseInt(e.target.value) })} className="w-full rounded border border-line px-3 py-2 bg-transparent text-ink focus:border-brand focus:outline-none" />
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <input type="text" value={formData.titleNe || ''} onChange={e => setFormData({ ...formData, titleNe: e.target.value })} className="w-full rounded border border-line px-3 py-2 bg-transparent text-ink focus:border-brand focus:outline-none" />
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <textarea required rows={4} value={formData.bodyEn || formData.body || ''} onChange={e => setFormData({ ...formData, bodyEn: e.target.value, body: e.target.value })} className="w-full rounded border border-line px-3 py-2 bg-transparent text-ink focus:border-brand focus:outline-none" />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-ink mb-1">Body (NE)</label>
                                    <textarea rows={4} value={formData.bodyNe || ''} onChange={e => setFormData({ ...formData, bodyNe: e.target.value })} className="w-full rounded border border-line px-3 py-2 bg-transparent text-ink focus:border-brand focus:outline-none" />
                                </div>
                            </div>
